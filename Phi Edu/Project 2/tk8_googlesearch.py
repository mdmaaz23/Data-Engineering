...
root = tk.Tk()
#title of window
root.title("Md's - Google Search Engine")
#window size
...
def search_query():
        ...
        s = googlesearch.search(query, tld="co.in", num=10, stop=10, pause=2)
        print(s)
        for j in s:
                print(j)
                webbrowser.open(j)   #it will open in chrome otherwise here only if it is commented
...
#Gmail label
g = Label(root,text="Gmail",bg="red",fg="white",cursor="hand2")
g.place(x=630,y=50)
g.bind("<Button-1>",lambda e: callback("https://mail.google.com/mail/"))

#Images label
i = Label(root,text="Images",bg="green",fg="white",cursor="hand2")
i.place(x=670,y=50)
...
apps.bind("<Button-1>",lambda e: callback("https://www.google.com/doodles/"))
